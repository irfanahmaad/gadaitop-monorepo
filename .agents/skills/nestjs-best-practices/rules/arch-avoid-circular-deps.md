...
Circular dependencies occur when Module A imports Module B, and Module B imports Module A (directly or transitively). Prefer extracting shared logic to a third module or using events to decouple. If circular imports are unavoidable (e.g. AuthModule ↔ UserModule), use `forwardRef(() => OtherModule)` in both modules; still aim to remove the cycle over time.
