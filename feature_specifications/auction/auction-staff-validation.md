...
The list is organized into three tabs: **Dijadwalkan** (batches in draft or pickup in progress), **Waiting for Approval** (batches in `validation_pending` — i.e. in validation phase, where staff perform Setuju/Tolak on items), and **Tervalidasi** (batches in `ready_for_auction` after staff finalize).
