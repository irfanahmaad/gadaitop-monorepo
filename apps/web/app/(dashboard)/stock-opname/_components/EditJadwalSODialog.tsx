...
import { useUpdateStockOpname } from "@/lib/react-query/hooks/use-stock-opname"
import type { StockOpnameSession, UpdateStockOpnameSessionDto } from "@/lib/api/types"
...
interface EditJadwalSODialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  onClose: () => void
  session: StockOpnameSession | null
  /** Called after successful update (e.g. to invalidate list queries) */
  onSuccess?: () => void
}

export function EditJadwalSODialog({
  open,
  onOpenChange,
  onClose,
  session,
  onSuccess,
}: EditJadwalSODialogProps) {
  ...
  // Reset and pre-populate form when dialog opens/closes
  React.useEffect(() => {
    if (open && session) {
      form.reset({
        id: session.uuid,
  ...
  const handleConfirmSubmit = async () => {
    if (!pendingValues || !ptId || !pendingValues.id) {
    ...
    try {
      ...
      toast.success("Jadwal SO berhasil diperbarui.")
      onSuccess?.()
      setConfirmOpen(false)
      setPendingValues(null)
      onClose()
    } catch {
      toast.error("Gagal memperbarui jadwal SO. Silakan coba lagi.")
    }
  }
